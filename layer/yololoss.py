...
class YoloLoss(nn.Module):
    ...
    def decode_loc(self,loc_preds):
        ...
        priorbox = Variable(PriorBox(cfg).forward()).cuda()
        loc_xy = loc_preds[:,:,:2,:,:]   #[N,5,2,13,13]
        grid_xy = priorbox[:,:2].view(fmsize,fmsize,2).permute(2,0,1)
        box_xy = torch.sigmoid(loc_xy)+grid_xy.expand_as(loc_xy)

        loc_wh = loc_preds[:,:,2:4,:,:]   #[N,5,2,13,13]
        anchor_wh = priorbox[:,2:4].view(fmsize,fmsize,2).permute(2,0,1)  #[2,13,13]
    ...
    def forward(self,preds,loc_targets,cls_targets,box_targets):
        ...
        pos = cls_targets.max(2).squeeze()>0       #[N,5,13,13]
        ...
        box_preds = self.decode_loc(preds[:,:,:4,:,:])   #[N,5,3,13,13]
